...
public class SkiPhone extends Activity implements PictureCallback {
  ...
  /**
   * This activity only runs a single task, so we need to update the intent
   * when this activity is resumed by the service.
   */
	@Override
  protected void onNewIntent(Intent intent) {
    ...
    // Don't set the screen orientation.
      ...
      // view the next time around.  Make sure the intent specifies the right
	    // mode.
    ...
    toggleButton.setText(isEnabled ? R.string.disable_button : 
      R.string.enable_button);	  
    ...
    addContentView(inflater.inflate(R.layout.camera_overlay, null),
        new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
    TextView textView = (TextView) findViewById(R.id.text_overlay);
    countdown = new Countdown(textView, 5,
        new Runnable() {
          public void run() {
            takePicture();
          }
	  ...
	  if (camera != null) {
	    ...
	    if (orientationListener.hasOrientation()) {
  	    ...
  	    try {
  	      ...
  	      // Oops, picture might not be oriented right, but at least it didn't
  	      // crash.
  ...
  public void onPictureTaken(byte[] data, Camera camera) {
    ...
    try {      
      ...
      // Write the image to the sdcard.
      File photoFile =
        new File(String.format("/sdcard/DCIM/SkiPhone/skiphone-%d.jpg",
    ...
    // The rating and feedback buttons should only be clickable if SkiPhone is
		// disabled.
    rateButton.setVisibility(isEnabled ? View.INVISIBLE : View.VISIBLE);
    feedbackButton.setVisibility(isEnabled ? View.INVISIBLE : View.VISIBLE);
